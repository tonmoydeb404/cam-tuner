/**
 * MediaPipe Tasks Vision person-segmentation adapter (extension-side).
 *
 ...
 * `@mediapipe/tasks-vision` runtime + local WASM that Center Stage already
 * ships, so there is no new dependency for this path.
 *
 * Like the face-detector adapter, this lives in the extension (not
 * stream-config) so it — and the heavy MediaPipe runtime — can be code-split
 * into a chunk loaded ON DEMAND only when a background effect is enabled.
 */
