/**
 * MediaPipe Tasks Vision person segmenter adapter (extension-side).
 *
 * Mirrors the face-detector adapter pattern: lives in the extension so it
 * (and the @mediapipe/tasks-vision runtime) can be code-split into a separate
 * ESM chunk loaded ON DEMAND only when background effects are enabled.
 *
