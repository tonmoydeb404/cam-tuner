/**
 * Robust Video Matting (RVM) adapter — the "high quality" backend (web preview).
 *
...
export type RvmMatteOptions = {
  ...
  modelUrl: string
  /** URLs/prefix for the ONNX Runtime WASM files (CDN for web). */
  wasmPaths?:
    | string
...
export async function createRvmMatteProvider(
  ...
  const downsampleRatio = new ort.Tensor("float32", [DSR], [1])
