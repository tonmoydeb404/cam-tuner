...
/**
 * Zoom behaviour for the controller.
 * - "auto"  — face size drives the zoom (within [minZoom, maxZoom]).
 * - "fixed" — zoom is left to the user's manual crop-zoom value; the controller
 *   only adjusts framing (alignCenter), if enabled.
 *
 * Zoom and framing are independent channels: either can be on without the
 * other, and they share a single face detector.
 */

export type CenterStageConfig = {
  /** Whether auto-framing (alignCenter panning) is active. */
  enabled: boolean
  ...
  /** EMA weight for blending raw detection into stabilised target (default 0.18). */
  ...
  maxZoom?: number
  /** Whether face size drives the zoom ("auto") or the user's manual zoom is
   *  used ("fixed"). Independent of `enabled`. Default "auto". */
  zoomMode?: ZoomMode
}
 ...
 * may hand a plugin: a raw `<video>`, a composited canvas/OffscreenCanvas
 * produced by another plugin's `prepareSource`, an ImageBitmap, or a
 * VideoFrame. Returns null when the dimensions aren't available yet (e.g. a
 * video before metadata has loaded).
 *
