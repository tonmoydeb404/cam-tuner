...
export function getSourceSize(source: CanvasImageSource): Size {
  ...
  if (
    ...
    if (s.displayWidth && s.displayHeight)
      return { width: s.displayWidth, height: s.displayHeight }
  }
  if (
    typeof s.naturalWidth === "number" &&
    typeof s.naturalHeight === "number"
  ) {
...
export type CropConfig = {
  ...
  /**
   * When true (default), the output keeps the original resolution and pads the
   * cropped region with `barColor` (letterbox). When false, the output is
   * resized to exactly the crop-box dimensions — pure crop, no bars.
   */
  letterbox?: boolean
}
